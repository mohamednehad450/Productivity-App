...
const ListRow: FC<ListRowProps> = ({
    ...
    () =>
      setHeight(
        rowRef.current.offsetHeight +
          1 +
          (expanded ? expRef.current.offsetHeight : 0)
      ),
    [expanded]
  );
  ...
  return (
    <div
      style={{ height }}
      className={`row-container ${expanded && expandedItem ? "gray-bg" : ""}`}
    >
      <div ref={rowRef} onClick={() => onClick && onClick()} className="row">
        <div className="row-section">{leftItem}</div>
        <div className="row-section">{rightItem}</div>
      </div>
      {expandedItem && (
        <div
          ref={expRef}
