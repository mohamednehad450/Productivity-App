...
export const updateTodo = async (
  ...
  const { data } = await Axios.patch<Todo>(`/api/todos/${id}/`, todo, {
    headers: {
      Authorization: `JWT ${user?.token}`,
    },
  });
...
export const checkStep = async (
  ...
  stepId: Step["id"],
  user?: User
): Promise<{ checked: Step["checked"] }> => {
  const { data } = await Axios.post<Todo>(
    `/api/todos/${id}/check_step/`,
    { stepId },
    {
      headers: {
        Authorization: `JWT ${user?.token}`,
...
export const checkTodo = async (
  id: Todo["id"],
  user?: User
): Promise<{ checked: Todo["checked"] }> => {
  const { data } = await Axios.post<{ checked: Todo["checked"] }>(
    `/api/todos/${id}/check_todo/`,
    {},
    {
      headers: {
        Authorization: `JWT ${user?.token}`,
